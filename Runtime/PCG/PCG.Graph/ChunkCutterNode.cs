...
namespace Chinchillada.Grid.PCGraphs
{
    ...
    public class ChunkCutterNode : IntGridModifierNode, IUsesRNG
    {
        ...
        [Input, ShowAsDrawer] public INeighborhoodFactory NeighborhoodFactory = new Diagonal.Factory();
        ...
        protected override IEnumerable<Grid2D<int>> Modify(Grid2D<int> grid)
        {
            ...
            var chunkMap = new ChunkMap(grid, this.empty, this.NeighborhoodFactory);
    ...
    public class ChunkMap
    {
        ...
        public ChunkMap(Grid2D<int> grid, int empty, INeighborhoodFactory neighborhoodFactory)
        {
            this.grid = grid;
            this.empty = empty;
        ...
        public List<Chunk> CutRandomSubChunk(Chunk chunk, int size, IRNG random)
        {
            ...
            {
                var connectionsToChunk = this.neighborhoodFactory.GetWithValue(this.map, coordinate, chunk.ID);

                foreach (Vector2Int connection in connectionsToChunk)
                {
                    this.ClearCoordinate(connection);

                    var remainders = this.neighborhoodFactory.GetWithValue(this.map, connection, chunk.ID);
        ...
        private Chunk GenerateRandomSubChunk(Chunk chunk, int size, IRNG random)
        {
            ...
            while (frontier.Any() && subChunk.Size < size)
            {
                ...
                var neighbors = this.neighborhoodFactory.GetWithValue(this.map, coordinate, chunk.ID);
        ...
        private Chunk CreateChunkAt(Vector2Int coordinate)
        {
            ...
            var coordinates = this.map.FloodFill(coordinate, canvas, chunkID, this.neighborhoodFactory);
